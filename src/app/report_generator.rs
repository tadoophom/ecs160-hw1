//! Report generation logic separated from data collection.
//! Handles calculation of metrics and summary generation.

use std::collections::HashMap;
use crate::model::Repo;
use crate::app::{RepoMetrics, LanguageReport};

/// Generates language reports and metrics calculations
pub struct ReportGenerator;

impl ReportGenerator {
    /// Calculates repository metrics including top files and commit counts
    pub fn summarize_repos(repos: &[Repo]) -> (Vec<RepoMetrics>, usize) {
        let mut metrics = Vec::with_capacity(repos.len());
        let mut fork_commit_total = 0usize;

        for repo in repos {
            let top_files = Self::compute_top_files(repo);
    ...
    /// Computes the top 3 most modified files in a repository
    ...
    pub fn create_language_report(language: &str, repos: Vec<Repo>) -> LanguageReport {
        ...
        let (repo_metrics, new_fork_commits) = Self::summarize_repos(&repos);
